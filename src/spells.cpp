...
bool Spells::registerEvent(Event_ptr event, const pugi::xml_node&)
{
	...
	if (instant) {
		...
		if (!result.second) {
			LOG_WARN_STREAM("Spells") << "[Warning - Spells::registerEvent] Duplicate registered instant spell with words: "
	...
	if (rune) {
		...
		if (!result.second) {
			LOG_WARN_STREAM("Spells") << "[Warning - Spells::registerEvent] Duplicate registered rune with id: "
...
bool Spells::registerInstantLuaEvent(InstantSpell* event)
{
	...
	if (instant) {
		...
		auto result = instants.emplace(instant->getWords(), std::move(*instant));
		if (!result.second) {
			LOG_WARN_STREAM("Spells") << "[Warning - Spells::registerInstantLuaEvent] Duplicate registered instant spell with words: "
			          << words << std::endl;
		}
		return result.second;
	}
...
bool Spells::registerRuneLuaEvent(RuneSpell* event)
{
	...
	if (rune) {
		...
		auto result = runes.emplace(rune->getRuneItemId(), std::move(*rune));
		if (!result.second) {
			LOG_WARN_STREAM("Spells") << "[Warning - Spells::registerRuneLuaEvent] Duplicate registered rune with id: " << id
			          << std::endl;
		}
		return result.second;
	}
...
bool CombatSpell::executeCastSpell(Creature* creature, const LuaVariant& var)
{
	...
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("Spells") << "[Error - CombatSpell::executeCastSpell] Call stack overflow" << std::endl;
		return false;
	}
...
bool Spell::configureSpell(const pugi::xml_node& node)
{
	...
	if (!nameAttribute) {
		LOG_ERROR_STREAM("Spells") << "[Error - Spell::configureSpell] Spell without name" << std::endl;
		return false;
	}
	...
	for (const char* reserved : reservedList) {
		if (caseInsensitiveEqual(reserved, name)) {
			LOG_ERROR_STREAM("Spells") << "[Error - Spell::configureSpell] Spell is using a reserved name: " << reserved << std::endl;
			return false;
		}
	}
	...
	if ((attr = node.attribute("group"))) {
		...
		if (tmpStr == "none" || tmpStr == "0") {
			...
			LOG_WARN_STREAM("Spells") << "[Warning - Spell::configureSpell] Unknown group: " << attr.as_string() << std::endl;
		}
	}
	...
	if ((attr = node.attribute("secondarygroup"))) {
		...
		if (tmpStr == "none" || tmpStr == "0") {
			...
			LOG_WARN_STREAM("Spells") << "[Warning - Spell::configureSpell] Unknown secondarygroup: " << attr.as_string() << std::endl;
		}
	}
	...
	if ((attr = node.attribute("blocktype"))) {
		...
		if (tmpStrValue == "all") {
			...
			LOG_WARN_STREAM("Spells") << "[Warning - Spell::configureSpell] Blocktype \"" << attr.as_string() << "\" does not exist."
			          << std::endl;
		}
	}
	...
	for (auto vocationNode : node.children()) {
		...
		if (vocationId != -1) {
			...
			LOG_WARN_STREAM("Spells") << "[Warning - Spell::configureSpell] Wrong vocation name: " << attr.as_string() << std::endl;
		}
	}
	return true;
}
...
bool InstantSpell::executeCastSpell(Creature* creature, const LuaVariant& var)
{
	// onCastSpell(creature, var)
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("Spells") << "[Error - InstantSpell::executeCastSpell] Call stack overflow" << std::endl;
		return false;
	}
...
bool RuneSpell::configureEvent(const pugi::xml_node& node)
{
	...
	if (!(attr = node.attribute("id"))) {
		LOG_ERROR_STREAM("Spells") << "[Error - RuneSpell::configureSpell] Rune spell without id." << std::endl;
		return false;
	}
...
bool RuneSpell::executeCastSpell(Creature* creature, const LuaVariant& var, bool isHotkey)
{
	...
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("Spells") << "[Error - RuneSpell::executeCastSpell] Call stack overflow" << std::endl;
		return false;
	}
