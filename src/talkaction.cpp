...
bool TalkAction::configureEvent(const pugi::xml_node& node)
{
	...
	if (!wordsAttribute) {
		LOG_STDOUT << "[Error - TalkAction::configureEvent] Missing words for talk action or spell" << std::endl;
		return false;
	}
...
bool TalkAction::executeSay(Player* player, const std::string& words, const std::string& param, SpeakClasses type) const
{
	...
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_STDOUT << "[Error - TalkAction::executeSay] Call stack overflow" << std::endl;
		return false;
	}
