...
bool ChatChannel::executeCanJoinEvent(const Player& player)
{
	...
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("Chat") << "[Error - CanJoinChannelEvent::execute] Call stack overflow" << std::endl;
		return false;
	}
...
bool ChatChannel::executeOnJoinEvent(const Player& player)
{
	...
	// onJoin(player)
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("Chat") << "[Error - OnJoinChannelEvent::execute] Call stack overflow" << std::endl;
		return false;
	}
...
bool ChatChannel::executeOnLeaveEvent(const Player& player)
{
	...
	// onLeave(player)
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("Chat") << "[Error - OnLeaveChannelEvent::execute] Call stack overflow" << std::endl;
		return false;
	}
...
bool ChatChannel::executeOnSpeakEvent(const Player& player, SpeakClasses& type, const std::string& message)
{
	...
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("Chat") << "[Error - OnSpeakChannelEvent::execute] Call stack overflow" << std::endl;
		return false;
	}
	...
	for (auto channelNode : doc.child("channels").children()) {
		...
		if (it != normalChannels.end()) {
			...
			if (scriptAttribute) {
				if (scriptInterface.loadFile("data/chatchannels/scripts/" + std::string(scriptAttribute.as_string())) ==
					...
					LOG_WARN_STREAM("Chat") << "[Warning - Chat::load] Can not load script: " << scriptAttribute.as_string()
					          << std::endl;
				}
			}
		...
		if (scriptAttribute) {
			if (scriptInterface.loadFile("data/chatchannels/scripts/" + std::string(scriptAttribute.as_string())) ==
				...
				LOG_WARN_STREAM("Chat") << "[Warning - Chat::load] Can not load script: " << scriptAttribute.as_string() << std::endl;
			}
		}
