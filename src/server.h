...
{
	if (port == 0) {
		LOG_STDOUT << "ERROR: No port provided for service " << ProtocolType::protocol_name() << ". Service disabled."
		          << std::endl;
		return false;
	}
	...
	if (foundServicePort == acceptors.end()) {
		...
		if (service_port->is_single_socket() || ProtocolType::server_sends_first) {
			LOG_STDOUT << "ERROR: " << ProtocolType::protocol_name() << " and " << service_port->get_protocol_names()
