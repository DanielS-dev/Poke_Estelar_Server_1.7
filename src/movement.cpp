	...
	if (it == map.end()) {
		...
		for (MoveEvent& existingMoveEvent : moveEventList) {
			if (existingMoveEvent.getSlot() == moveEvent.getSlot()) {
				LOG_WARN_STREAM("Movement") << "[Warning - MoveEvents::addEvent] Duplicate move event found: " << id << std::endl;
			}
		}
...
void MoveEvents::addEvent(MoveEvent moveEvent, const Position& pos, MovePosListMap& map)
{
	...
	if (it == map.end()) {
		...
		std::list<MoveEvent>& moveEventList = it->second.moveEvent[moveEvent.getEventType()];
		if (!moveEventList.empty()) {
			LOG_WARN_STREAM("Movement") << "[Warning - MoveEvents::addEvent] Duplicate move event found: " << pos << std::endl;
		}
...
std::string_view MoveEvent::getScriptEventName() const
{
	switch (eventType) {
		...
		case MOVE_EVENT_REMOVE_ITEM:
			return "onRemoveItem";
		default:
			LOG_ERROR_STREAM("Movement") << "[Error - MoveEvent::getScriptEventName] Invalid event type" << std::endl;
			return "";
	}
}
...
bool MoveEvent::configureEvent(const pugi::xml_node& node)
{
	...
	if (!eventAttr) {
		LOG_ERROR_STREAM("Movement") << "[Error - MoveEvent::configureMoveEvent] Missing event" << std::endl;
		return false;
	}
	...
	if (tmpStr == "stepin") {
		...
		LOG_INFO_STREAM("Movement") << "Error: [MoveEvent::configureMoveEvent] No valid event name " << eventAttr.as_string()
		          << std::endl;
		return false;
	}
	...
	if (eventType == MOVE_EVENT_EQUIP || eventType == MOVE_EVENT_DEEQUIP) {
		...
		if (slotAttribute) {
			...
			if (tmpStr == "head") {
				...
				LOG_WARN_STREAM("Movement") << "[Warning - MoveEvent::configureMoveEvent] Unknown slot type: "
...
bool MoveEvent::loadFunction(const pugi::xml_attribute& attr, bool isScripted)
{
	...
	if (caseInsensitiveEqual(functionName, "onstepinfield")) {
		...
		if (!isScripted) {
			LOG_WARN_STREAM("Movement") << "[Warning - MoveEvent::loadFunction] Function \"" << functionName << "\" does not exist."
			          << std::endl;
			return false;
		}
	}
...
bool MoveEvent::executeStep(Creature* creature, Item* item, const Position& pos)
{
	...
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("Movement") << "[Error - MoveEvent::executeStep] Call stack overflow" << std::endl;
		return false;
	}
...
bool MoveEvent::executeEquip(Player* player, Item* item, slots_t slot, bool isCheck)
{
	...
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("Movement") << "[Error - MoveEvent::executeEquip] Call stack overflow" << std::endl;
		return false;
	}
...
bool MoveEvent::executeAddRemItem(Item* item, Item* tileItem, const Position& pos)
{
	...
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("Movement") << "[Error - MoveEvent::executeAddRemItem] Call stack overflow" << std::endl;
		return false;
	}
