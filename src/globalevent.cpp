...
bool GlobalEvents::registerEvent(Event_ptr event, const pugi::xml_node&)
{
	...
	LOG_WARN_STREAM("GlobalEvents") << "[Warning - GlobalEvents::configureEvent] Duplicate registered globalevent with name: "
...
void GlobalEvents::think()
{
	...
	for (auto& it : thinkMap) {
		...
		if (!globalEvent.executeEvent()) {
			LOG_ERROR_STREAM("GlobalEvents") << "[Error - GlobalEvents::think] Failed to execute event: " << globalEvent.getName()
			          << std::endl;
		}
...
bool GlobalEvent::configureEvent(const pugi::xml_node& node)
{
	...
	if (!nameAttribute) {
		LOG_ERROR_STREAM("GlobalEvents") << "[Error - GlobalEvent::configureEvent] Missing name for a globalevent" << std::endl;
		return false;
	}
	...
	if ((attr = node.attribute("time"))) {
		...
		if (hour < 0 || hour > 23) {
			LOG_ERROR_STREAM("GlobalEvents") << "[Error - GlobalEvent::configureEvent] Invalid hour \"" << attr.as_string()
		...
		if (params.size() > 1) {
			min = params[1];
			if (min < 0 || min > 59) {
				LOG_ERROR_STREAM("GlobalEvents") << "[Error - GlobalEvent::configureEvent] Invalid minute \"" << attr.as_string()
				          << "\" for globalevent with name: " << name << std::endl;
				return false;
			}
			...
			if (params.size() > 2) {
				sec = params[2];
				if (sec < 0 || sec > 59) {
					LOG_ERROR_STREAM("GlobalEvents") << "[Error - GlobalEvent::configureEvent] Invalid second \"" << attr.as_string()
					          << "\" for globalevent with name: " << name << std::endl;
					return false;
				}
			}
		...
		if (caseInsensitiveEqual(value, "startup")) {
			...
			LOG_ERROR_STREAM("GlobalEvents") << "[Error - GlobalEvent::configureEvent] No valid type \"" << attr.as_string()
			          << "\" for globalevent with name " << name << std::endl;
			return false;
		}
		...
		LOG_ERROR_STREAM("GlobalEvents") << "[Error - GlobalEvent::configureEvent] No interval for globalevent with name " << name
		          << std::endl;
		return false;
	}
	return true;
...
bool GlobalEvent::executeRecord(uint32_t current, uint32_t old)
{
	...
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("GlobalEvents") << "[Error - GlobalEvent::executeRecord] Call stack overflow" << std::endl;
		return false;
	}
...
{
	if (!tfs::lua::reserveScriptEnv()) {
		LOG_ERROR_STREAM("GlobalEvents") << "[Error - GlobalEvent::executeEvent] Call stack overflow" << std::endl;
		return false;
	}
