...
bool Scripts::loadScripts(std::string folderName, bool isLib, bool reload)
{
	...
	if (!fs::exists(dir) || !fs::is_directory(dir)) {
		LOG_WARN_STREAM("Scripts") << "[Warning - Scripts::loadScripts] Can not load folder '" << folderName << "'." << std::endl;
		return false;
	}
		...
		if (fs::is_regular_file(*it) && it->path().extension() == ".lua") {
			...
			if (found != std::string::npos) {
				if (getBoolean(ConfigManager::SCRIPTS_CONSOLE_LOGS)) {
					LOG_DEBUG_STREAM("Scripts") << "> " << it->path().filename().string() << " [disabled]" << std::endl;
				}
				continue;
			}
			v.push_back(it->path());
	...
	for (auto it = v.begin(); it != v.end(); ++it) {
		...
		if (!isLib) {
			if (redir.empty() || redir != it->parent_path().string()) {
				...
				if (getBoolean(ConfigManager::SCRIPTS_CONSOLE_LOGS)) {
					LOG_INFO_STREAM("Scripts") << ">> [" << p.parent_path().filename() << "]" << std::endl;
				}
		...
		if (scriptInterface.loadFile(scriptFile) == -1) {
			LOG_ERROR_STREAM("Scripts") << "> " << it->filename().string() << " [error]" << std::endl;
			LOG_ERROR_STREAM("Scripts") << "^ " << scriptInterface.getLastLuaError() << std::endl;
			continue;
		}

		if (getBoolean(ConfigManager::SCRIPTS_CONSOLE_LOGS)) {
			if (!reload) {
				LOG_DEBUG_STREAM("Scripts") << "> " << it->filename().string() << " [loaded]" << std::endl;
			} else {
				LOG_DEBUG_STREAM("Scripts") << "> " << it->filename().string() << " [reloaded]" << std::endl;
			}
		}
	}
